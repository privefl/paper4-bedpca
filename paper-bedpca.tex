...
\abstract{
Principal Component Analysis (PCA) of genetic data is routinely used to infer ancestry and control for population structure in various genetic analyses. However, conducting PCA in large populations is complicated, and there are several potential pitfalls. These pitfalls include (1) capturing Linkage Disequilibrium (LD) structure instead of population structure, (2) projected PCs that suffer from shrinkage bias when projecting PCA from a reference dataset to another independent dataset, and (3) detecting sample outliers. In this work, we explore these pitfalls of using PCA, and present efficient solutions to these pitfalls. Following applications to the UK Biobank and the 1000 Genomes project datasets, we make recommendations for best practices and provide efficient and user-friendly implementations of the proposed solutions in R packages bigsnpr and bigutilsr.

For example, we show that PC19 to PC40 in the UK Biobank are capturing LD structure. Using our automatic algorithm for removing long-range LD region, we are able to recover only 16 PCs that capture population structure only.
Therefore, we recommend using only 16-18 PCs from the UK Biobank. Another example: when projecting the PCA computed on the 1000 Genomes project, although PC1 to PC4 suffer from only moderate shrinkage (1.00-1.08), PC5 for example suffers from a shrinkage factor of 1.51 and PC10 from a shrinkage factor of 3.16. We provide a fast way to project new individuals and that is not affected by this shrinkage bias. We also show how to restrict to individuals that are genetically homogeneous based on PCA. 
...
This is now possible to compute PCA on these very large datasets thanks to software such as FastPCA (fast mode of EIGENSOFT), FlashPCA2, PLINK 2.0 (approx mode), bigstatsr/bigsnpr and TeraPCA \cite[]{galinsky2016fast,abraham2017flashpca2,chang2015second,prive2017efficient,bose2019terapca}.
...
An overview of existing methods with their respective pros and cons is presented in table \ref{tab:method-overview}.

For this paper, we derive a new implementation of PCA that can be used directly on bed/bim/fam PLINK files with some missing values, which we make available in a new release of R package bigsnpr (v1.0.0). Then, we recall some of the pitfalls of PCA computation on genotype data and provide efficient solutions to these issues, which we implement in R packages bigsnpr and bigutilsr. These includes accouting for LD structure in PCA, detecting outliers in PC scores and projecting PCs to another study sample without shrinkage bias.

[WHERE TO PUT THAT WE GET 40 PCs WHEN SUBSETTING?]
...
\hline
Analysis & Method and/or Software & Pros & Cons \\
\hline
\multirow{3}{*}{PCA} & bigstatsr / bigsnpr \cite[]{prive2017efficient} & \makecell{Fast and accurate + handle dosages \\ + thinning options directly included} & \makecell{Own format without missing values \\ (fast functions are available \\ for converting and imputing)} \\
 ...
 & \makecell{PLINK 2.0 (reimplementation of FastPCA) \\ \cite[]{galinsky2016fast,chang2015second}} & Fast & \makecell{Possible lack of accuracy \\ \cite[]{abraham2017flashpca2,prive2017efficient}} \\
\hline
\makecell{Detection of \\ oultier samples} & \makecell{``6 SDs from the mean'' \\ in EIGENSOFT \cite[]{patterson2006population}} & Simple & Assume Gaussian distritbution \\
\hline
...
\multirow{5}{*}{\makecell{Projection of \\ new individuals \\ onto reference \\ PCA space}} & Simple projection (multiplication by loadings) & Simple & Shrinkage biased \\
  & Bias-adjusted projection in R package hdpca \cite[]{dey2019asymptotic} & Independent of new samples & \makecell{Assume same shrinkage for all individuals \\ + model-based \\ + need all eigenvalues of reference} \\
  & \makecell{Augmentation, Decomposition and Procrustes transformation \\ (ADP) in LASER 2.0 \cite[]{wang2015improved}} & Accurate & Slow (a new PCA for each new sample) \\
  & Online ADP (OADP) in python package FRAPOSA \cite[]{zhang2019fast} & Much faster than ADP & \makecell{Does not work for related individuals} \\ 
\hline
\end{tabular}
\end{table}
\endgroup
...
To compute this decomposition easily and efficiently, we implement an accessor that memory-map the bed file to use it directly as if it were a standard matrix. Then, we apply the same algorithm for partial SVD that is used in R package bigstatsr \cite[]{prive2017efficient}.

\subsection{Robust Mahalanobis distance \label{maha}}

In this paper, for many applications, we use the pairwise orthogonalised Gnanadesikan-Kettenrin robust Mahalanobis distance as implemented in function \texttt{covRob} of R package robust with parameter \texttt{estim = "pairwiseGK"} \cite[]{gnanadesikan1972robust,yohai1988high,maronna2002robust,todorov2009object}. We reexport function \texttt{covRob} in R package bigutilsr for convenience.
...
One assumption of Tukey's rule is that the sample is normally distributed; we account for skewness in the data using the medcouple \cite[]{hubert2008adjusted}. 
...
We implement this statistic in function \texttt{prob\_dist} of R package bigutilsr. To make it fast, we use the fast K nearest neighbours implementation of R package nabor \cite[]{elseberg2012comparison} and paralleliseit in function \texttt{knn\_parallel} of package bigutilsr.
...
In this paper, we also analyse the genotyped version of the UK Biobank \cite[]{bycroft2017genome}.
...
\noindent[MOVE FIGURE TO MAIN TEXT? E.G. \ref{fig:UKBB-scores2}]
...
Finally, it takes only 6 minutes to compute the 30-Nearest Neighbours of 20 PC scores for 406,545 UK Biobank individuals, which is the main computational aspect of computing statistics for detecting individual outlier samples (Section \ref{outlier-sample}).

\subsection{Outlier sample detection}

To detect a few sample outliers, we compare the rule of ``6 SDs from the mean'' (6SD) to the statistic we propose in section \ref{outlier-sample}.
...
6SD identifies a lot of outliers, of which some are part of a relatively large cluster (Figure \ref{fig:outlier-sd}).
[WHERE TO PUT FOLLOWING?] In theory, if all PCs are normally distributed, after correcting for multiple testing of 2500 individuals and 20 PCs, 6SD has a small probability of 0.0001 of detecting one outlier or more.
...
Here, we choose a threshold of 5 and show that this gives a similar subset of individuals than the ``White British'' subset reported by the UK Biobank (Figure \ref{fig:homogeneous}).
Using this threshold of 5, only 3 out of 10,936 people of Asian ancestry (1 ``Chinese'' and 2 ``Indian'') are kept, and 1 ``African'' out of 7622 poeple with Black background is kept (Table \ref{tab:homogeneous}). 
...
We use 60\% of individuals in the 1000G data (section \ref{1000G}) to compute the PCA ($K=20$ PCs). Then, we project the remaining 40\% individuals using three methods: 1/ simply multiplying the genotypes of these individuals by the previously computed loadings, 2/ correcting the simple projections using asymptotic shrinkage factors as determined by R package hdpca v1.1.3 \cite[]{dey2019asymptotic}, with all eigenvalues derived from the Genetic Relationship Matrix (GRM) computed with function \texttt{bed\_tcrossprodSelf} of R package bigsnpr, and 3/ the OADP projection (section \ref{proj}). 
...
When correcting projected PC scores with asymptotic shrinkage factors, bias is smaller than when simply projecting loadings, yet, there is an apparent bias for PC7-8 (Figure \ref{fig:proj1000G-3}).
...
Principal Component loadings reported by the UK Biobank clearly show that PC19 to PC40 capture LD structure, which is also the case for PC16 and PC18, although less pronounced (see peaks in figure \ref{fig:UKBB-loadings40}). 
When applying our automatic procedure to remove long-range LD regions, it did not converge after 5 iterations, meaning that it kept detecting long-range LD regions at each iteration. Therefore, we were able to capture only 16 PCs that shows population stratification and not LD structure (Figures \ref{fig:UKBB-screeplot}, \ref{fig:UKBB-scores} and \ref{fig:UKBB-loadings}).

Then, we computed again PCA after restricting the individuals included in the computations: we randomly subsampled UKBB data to use only 10,000 British individuals (out of 431,029) and 5000 Irish individuals (out of 12,755), while keeping all individuals with other or unkonwn self-reported ancestry. 
...
We show that we are able to capture more PCs (at least 40) that display visual population structure (Figures \ref{fig:UKBB-scores2} and \ref{fig:UKBB-screeplot2}).
...
\caption{Pipeline for computing Principal Components (PCs) using R packages bigsnpr and bigutilsr. [ADD VISUAL INSPECTION? QC? IN THE LEGEND?]
\label{fig:pipeline}}
\end{figure}

In this work, we have compiled different pitfalls that can occur with analyses related to Principal Component Analysis (PCA) of genetic data. Then, we have investigated possible solutions to these pitfalls and chose the ones that we thought to be the most relevant both with respect to properties such as accuracy and robustness, but also that would be fast and easy to use. We then implemented these solutions in open-source R packages. 
These functions can be directly used on genotypes stored as PLINK bed/bim/fam files with some missing values. 
This is in contrast with our first release of packages bigstatsr and bigsnpr that use their own ``bigSNP'' format that can store both genotype calls and (rounded) dosages, but that mostly provide functions that require no missing value.
Yet, PCA is a useful tool on its own and does not require extensive imputed data, therefore we decided that operating directly on PLINK files with a few missing values would be more practical for the users.

As a general recommendation for computing PCA, we show some pipeline in figure \ref{fig:pipeline}. [START WITH QC?]
This requires removing all related individuals, for which we provide an R wrapper to PLINK's implementation of KING robust kinship coefficients \cite[]{manichaikul2010robust,chang2015second}. 
Note that one should remove both individuals in each related pairs so that projected individuals at the end are not related to the ones used for computing PCA, as we showed that relatedness is a problem when using the OADP projection.
Then, it requires to do several steps of outlier detection, one for outlier variants that capture long-range LD variation (automatic), and one for detecting outlier samples (semi-automatic and visual).
To fasten these steps, we tried hard to come up with a solution for not recomputing PCA from scratch when removing a few samples or variants. Using educated guesses based on low-rank approximations seemed to be a promising approach but did not fasten computation by much, so we did not pursue this idea \cite[]{brand2003fast,wu2017primme}. 
Once PCA is done, one should look at the loadings to make sure that there is no apparent LD structure left. 
One should also look at PC scores to identify which PCs capture population structure.
As in many applications, we believe that a compromise should be made between signal and noise. Therefore, we recommend to use only PCs that show population structure (e.g.\ first 16 PCs in figure \ref{fig:UKBB-scores}) [NOTE SURE ABOUT PC15-16..].
...
We show that it is useful to actually subset these individuals so there is not too much imbalance between the number of people from different ancestries. 
Indeed, when subsetting British and Irish people, we are able to capture a lot more PCs that show population structure.
This makes designs such as the 1000 Genomes project, that gathered around 100 people for each of 26 populations, highly relevant for capturing population structure.
...
When the homogeneous sample is not predominant in the dataset, one solution is to compute the center and covariance of the robust Mahalanobis distance using only the population of interest, and then computing the distances for all individuals using these parameters. 

Finally, we have showed some pitfalls when computing PCA on genotype data, yet we think most of the results in this paper are not inherent to genotype data, and can be transferred to e.g.\ other omics data as well. For example, outlier and homogeneous sample detection can be used on PCs of anything. Moreover, projection of scores will also be a problem for other omics data where the number of variables used is larger than the number of samples used for computing PCA. Finally, our recommendation of gathering ``populations'' of approximately the same size is relevant for other biological data as well.
...
\noindent[DISCUSS ABOUT PC16 -> FROM DIFFERENT ASSESSMENT CENTRES -> 36\% correlation with number of NA for all PC16 > 30]

\noindent[MORE CITATIONS IN THE DISCUSSION?]
...
A tutorial on the steps to perform PCA on 1000G data is presented at \url{} [TODO]. 
...
\caption{Principal Component (PC) scores 13 to 20 of 1000G, colored by statistic of outlierness.\\~\\}
\end{subfigure}
~~~~~~
\begin{subfigure}[b]{0.45\textwidth}
\includegraphics[width=\textwidth]{be-outlier-1000G.pdf}
\caption{Principal Component (PC) scores 13 to 20 of 1000G, colored by being detected as an outlier. Threshold of outlierness is determined based on histogram (a).}
\end{subfigure}
...
\caption{Principal Component (PC) scores 13 to 20 of 1000G, colored by statistic of outlierness (log-scale).\\~\\}
\end{subfigure}
~~~~~~
\begin{subfigure}[b]{0.45\textwidth}
...
\caption{PCs 1 to 20 of UKBB, colored by statistic of robust Mahalanobis distances (log-scale).\\}
\end{subfigure}
\\~\\
\begin{subfigure}[b]{0.45\textwidth}
...
\caption{PCs 1 to 20 of UKBB, colored by being detected as an outlier. Threshold of outlierness is determined based on histogram (c), where the threshold of 5 is chosen for the logarithm of the distances.}
\end{subfigure}
...
\begin{table}[ht]
\centering
\caption{Number of UKBB individuals with (log) Mahalanobis distance lower than some threshold (top), grouped by self-reported ancestry (left).} 
...
\caption{[SHOULD BE MAIN FIGURE?] Principal Component (PC) scores 27 to 50 computed on the UK Biobank using 48,942 individuals of diverse ancestries. Different colors represent different self-reported ancestries.
