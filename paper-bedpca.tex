...
In this work, we compiled different pitfalls that can occur with analyses related to Principal Component Analysis (PCA) of genetic data. We then investigated possible solutions to these pitfalls and choose the ones that we thought to be the most relevant both with respect to properties such as accuracy and robustness, but also that are fast and easy to use. We then implemented these solutions in open-source R packages that are available for anyone to use.

As a general recommendation for computing PCA, we show some pipeline in figure \ref{fig:pipeline}. [DESCRIBE ALL STEPS?]

\noindent[LIMITATION 1: RUN MULTIPLE TIMES / FAIL TO SPEED UP / PRIMME + LOW-RANK UPDATE (educated guesses)]
...
\noindent[OK TO PROJECT DIRECTLY UKBB (BECAUSE OF SIZE) -> INCLUDE? DO NOT CARE IF SUBSETTING]

\noindent[PROMOTE VISUAL INSPECTION]

\noindent[To make sure within one population. Do the Maha in one pop and then apply it to everyone]

\noindent[Show pipeline to compute PCA (diagram like first paper)]
