...
To compute this decomposition easily and efficiently, we implement an accessor that memory-map the bed file to use it directly as if it were a standard matrix. Then, we apply the same algorithm for partial SVD that is used in R packages bigstatsr and FlashPCA2, namely the implicitly restarted Arnoldi method \cite[]{Lehoucq1996,abraham2017flashpca2,prive2017efficient}. This method is implemented in R package RSpectra, and requires a function that computes the matrix-vector multiplication of the scaled genotyped matrix with a given vector, which we implement for bed files and parallelise.
...
When computing the PCA on the UK Biobank using 406,545 unrelated individuals and 171,977 variants, and projecting the 1000G data onto this reference PCA space, shrinkage is much smaller ($\leq 1.08$ for all 20 first PCs, Figure \ref{fig:proj1000G-6}). Shrinkage increases with an increased number of variants used, but decreases with an increased sample size (Table \ref{tab:shrinkage}).

\begingroup
\renewcommand*{\arraystretch}{1.8}
\begin{table}[h]
\centering
\caption{Shrinkage coefficients when projecting new individuals onto reference PCA space. We list the dataset, the sample size and number of variants used to compute the final truncated PCA.} 
\label{tab:shrinkage}
\begin{tabular}{|c|c|c|c|}
\hline
Dataset & Sample size (x1000) & \makecell {Number of variants \\ (x1000, after LD removal)} & Shrinkage (PC 1 - 5 - 10 - 20) \\
\hline
1000G & 1.5 & 393 & 1.01 - 1.50 - 3.14 - 6.70 \\
1000G & 2.5 & 229 & 1.01 - 1.36 - 2.84 - 6.75 \\
UKBB & 49 & 282 & 1.00 - 1.04 - 1.12 - 1.43 \\
...
Note that one should remove both individuals in each related pairs so that the projected individuals are not related to the ones used for computing PCA, as we showed that relatedness is a problem when using the OADP projection.
...
To fasten these steps, we explored solutions for not recomputing PCA from scratch when removing a few samples or variants. Using educated guesses in R package PRIMME based on low-rank approximations seemed to be a promising approach but did not fasten computation by much, so we did not pursue this idea \cite[]{brand2003fast,wu2017primme}. 
Once PCA is done, one should look at the loadings to make sure that there is no apparent LD structure left (peaks in PC loadings as shown in figures \ref{fig:UKBB-loadings40} and \ref{fig:UKBB-loadings}). 
Including PCs that capture LD as covariates in genetic analyses can lead to reduced power, e.g.\ for detecting genetic association within these LD regions in GWAS.  
One should also look at PC scores to identify which PCs capture population structure.
As in many applications, we believe that a compromise should be made between signal and noise. Therefore, we recommend to use only PCs that show structure that is not LD (e.g.\ first 16 PCs in figure \ref{fig:UKBB-scores}).
...
\noindent[NEED MORE CITATIONS IN THE DISCUSSION]

\noindent[WHAT IS PC16 IN FIG \ref{fig:UKBB-scores}?]
