...
To project PCs of a reference dataset (e.g.\ 1000G) to a target genotype data, we implement the following 3 steps in function \texttt{bed\_projectPCA} of package bigsnpr:
\begin{enumerate}
...
\item projecting computed PCs to the target data using an optimised implementation of the Online Augmentation, Decomposition, and Procrustes (OADP) transformation \cite[]{zhang2019fast}, which we describe in Supplementary Materials.
\end{enumerate}

%For step 4, we implement a shrinkage-bias adjustment method that combines the two methods proposed by \cite{zhang2019fast}. First, for only a subset of target individuals, we compute the Online Augmentation, Decomposition, and Procrustes (OADP) transformation (using $K''=K'=K$ [REMOVE?]). For these individuals, we regress the new PC scores resulting from OADP by the projected PC scores without adjustement; this gives us shrinkage-adjustment factors that we use to correct projected PC scores for all target individuals.
...
We use 60\% of individuals in the 1000G data (section \ref{1000G}) to compute the PCA ($K=20$ PCs). Then, we project the remaining 40\% individuals using two methods: 1/ simply multiplying the genotypes of these individuals by the previously computed loadings, and 2/ the OADP projection (section \ref{proj}). 
...
\item Get the eigen decomposition $Q^T Q = V' {\Delta'}^2 {V'}^T$ (truncated to $K$ components out of the $K+1$).
\item Calculate $$\begin{bmatrix} \widetilde{U} \\ \widetilde{u}  \end{bmatrix} = \begin{bmatrix} U & 0 \\ 0 & 1 \end{bmatrix} V' = \begin{bmatrix} U V'[1{:}K,~] \\ V'[K{+}1,~] \end{bmatrix} $$
\item Find the Procrustes transformation from $\widetilde{U}$ to $U$. As $\widetilde{U}$ and $U$ have both their columns centered already, the Procrustes transformation $\rho ~\widetilde{U} A$, where $\rho$ is a scaling coefficient and $A$ is an orthonomal projection matrix that minimise the Frobenius norm of $(\rho \widetilde{U} A - U)$, is given by $A = V'' {U''}^T$ and $\rho = \frac{\text{trace}(\Delta'')}{\text{trace}(\widetilde{U}^T \widetilde{U})}$ where $U'' \Delta'' {V''}^T$ is the full SVD of $U^T \widetilde{U} = V'[1{:}K,~]$ \cite[]{wang2015improved}. Then, $\rho = \frac{\text{trace}(\Delta'')}{\text{trace}(\widetilde{U}^T \widetilde{U})} = \frac{\text{trace}(\Delta'')}{\text{trace}({V'[1{:}K,~]}^T V'[1{:}K,~])} = \frac{\text{trace}(\Delta'')}{\text{trace}(\Delta''^2)}$. 
...
\begin{figure}[!htpb]
\centerline{\includegraphics[width=0.8\textwidth]{proj1000G-PC9-20.pdf}}
