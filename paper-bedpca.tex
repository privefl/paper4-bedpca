...
\noindent$^{\text{\sf 5}}$Laboratoire TIMC-IMAG, UMR 5525, Univ.\ Grenoble Alpes, CNRS, La Tronche, 38700, France. \\
...
For example, we show that PC19 to PC40 in the UK Biobank capture LD structure. Using our automatic algorithm for removing long-range LD region, we recover 16 PCs that capture population structure only.
Therefore, we recommend using only 16-18 PCs from the UK Biobank. 
We provide evidence for a shrinkage bias when projecting PCs computed with data from the 1000 Genomes project.  Although PC1 to PC4 suffer from only moderate shrinkage (1.01-1.09), PC5 for example suffers from a shrinkage factor of 1.50 and PC10 from a shrinkage factor of 3.14. 
We provide a fast way to project new individuals and that is not affected by this shrinkage bias. 
...
Principal Component Analysis (PCA) has been widely used in genetics for many years and in many contexts. For instance, adding PCs as covariates has been routinely used to adjust for population structure in Genome-Wide Association Studies (GWAS) \cite[]{price2006principal}. PCA has also been used to detect loci under selection \cite[]{galinsky2016fast,luu2017pcadapt}.
...
For this paper, we derive implementations of truncated PCA and other useful functions for e.g.\ performing LD thinning and computing variant statistics. We make these available in a new release of R package bigsnpr (v1.0.0); this package differs from previously available functions presented in \cite{prive2017efficient} is that these new functions can be used directly on bed/bim/fam PLINK files with some missing values. 
...
Finally, to choose the threshold on the previously described statistics above which to be considered as outlier, we use a modified version of Tukey's rule, a standard rule for detecting outliers \cite[]{tukey77}. 
...
Automatic outlier detection is difficult, therefore we recommend users to choose a threshold for this statistic to define outliers based on visual inspection (using the histogram of these statistics and the PC scores colored by these statistics).
...
In theory, if all PCs are normally distributed, after correcting for multiple testing of 2500 individuals and 20 PCs, 6SD has a small probability of 0.0001 of detecting one outlier or more.
...
\caption{Principal Component (PC) scores 13 to 20 of 1000G, colored by the statistic used to define outliers.\\~\\}
\end{subfigure}
~~~~~~
\begin{subfigure}[b]{0.45\textwidth}
...
When computing the PCs on the UK Biobank using 406,545 unrelated individuals and 171,977 variants, and projecting the 1000G data onto this reference PCA space, shrinkage is much smaller ($\leq 1.08$ for all 20 first PCs, Figure \ref{fig:proj1000G-6}). Shrinkage increases with an increased number of variants used, but decreases with an increased sample size (Table \ref{tab:shrinkage}).

\begingroup
\renewcommand*{\arraystretch}{1.8}
\begin{table}[h]
\centering
\caption{Shrinkage coefficients when projecting new individuals onto reference PCA space. We list the dataset, the sample size and number of variants used to compute the final truncated PCA. As expected, the shrinkage bias only becomes negligible if the PCA is conducted on large samples.} 
\label{tab:shrinkage}
...
As in many applications, we believe that a compromise should be made between signal and noise. Therefore, we recommend using only PCs that show structure (e.g.\ PC1-16 in figure \ref{fig:UKBB-scores}) and exclude PCs that indicate LD (e.g.\ PC19-40 in figure \ref{fig:UKBB-loadings40}).
...
\noindent[TODO: INCLUDE CITATIONS OF EVEN SAMPLING IN PCA]
...
\caption{Principal Component (PC) scores 13 to 20 of 1000G, colored by statistic used to detect outliers (log-scale).\\~\\}
\end{subfigure}
~~~~~~
\begin{subfigure}[b]{0.45\textwidth}
\includegraphics[width=\textwidth]{be-outlier2-1000G.pdf}
\caption{Principal Component (PC) scores 13 to 20 of 1000G, colored by being detected as an outlier. Threshold of being considered as an outlier is determined based on histogram (a).}
\end{subfigure}
