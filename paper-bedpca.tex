...
\noindent[ADD: implement for bed files with missing values (as opposed to a complete analysis with bigstatsr/bigsnpr), because useful on its own (and can be use only genotyped SNPs).]
...
To capture consecutive outliers that corresponds to long-range LD regions, we apply a Gaussian smoothing to these statistics.
...
Standard Tukey's rule also uses a fixed coefficient (1.5) that does not account for multiple testing, which means that for large samples, there are always some outliers detected when using 1.5. 
...
\noindent[TODO: Make a tuto with 1000G]
...
\noindent[TODO SECTION ON SAMPLE OUTLIER REMOVAL / COMPARE WITH EIGENSOFT]

\noindent[TODO SECTION ON HOMOGENEOUS SAMPLES / COMPARE WITH ABERRANT]

\noindent[SHOW WHAT HAPPENS WHEN NOT REMOVING RELATEDNESS IN PCA?]

\noindent[MORE EMPHASIS ON COMPUTING ALL EIGEN WITH BED\_TCROSSPRODSELF()?]
...
Our statistic identifies only isolated samples or isolated pairs that seems to be outliers driving structure of PC17-20 (Figure \ref{fig:outlier-pd}).
...
To restrict to homogeneous samples, we compare the use of R package Aberrant as used for UKBB to the use of the robust Mahalanobis distance we propose.
