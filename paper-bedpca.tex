...
Note that you can also use function \texttt{bed\_projectPCA} to project individuals from the reference dataset that were not used during the PCA computation (e.g.\ related people). 
...
Black points are the 60\% individuals used for computing PCA.
Green points are the 40\% remaining individuals, projected by simply multiplying their genotypes by the corresponding PC loadings, corrected using theoritical asymptotic shrinkage factors.
Blue points are the 40\% remaining individuals, projected using the Online Augmentation, Decomposition, and Procrustes (OADP) transformation.
