...
Method: Robust Mahalanobis distance for each reference population    
\+ minimum distance (or maximum p-value).

```{r, out.width="65%"}
knitr::include_graphics("figures/us.svg")
```
...
Matching POPRES populations to 1000G populations    
(do not match maximum p-values < 0.05):


```{r, out.width="90%"}
knitr::include_graphics("figures/ancestry-popres.png")
```
...
Matching UKBB populations to 1000G populations (20 minutes)  
(do not match maximum p-values < 0.05):

```{r, out.width="98%"}
knitr::include_graphics("figures/ancestry-UKBB.png")
```
